...
from utils import timing_decorator, logging_decorator, logger
import os
...
class DatabaseManager:
    ...
    def insert_articles(self, articles_data: List[Dict[str, Any]]):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for article in articles_data:
                ...
                cursor.execute("""
                    INSERT INTO Articles (title, content, source_id, date, category, sentiment_score)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (
                    article['title'], 
                    article['content'], 
    ...
    def export_to_json(self, output_path: str = "articles.json"):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT a.title, a.content, s.name as source, a.date, a.category, a.sentiment_score
                FROM Articles a
